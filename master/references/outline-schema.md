...
> **三个追踪列表的职责边界**
> - **理解债务**：学生主动声称"懂了"但未经验证（主动声称 + 未验证）
> - **搁置卡点**：所有类型（A/B/C/D）的暂时跳过卡点，包括前置概念"本主题学过但未掌握"的情况（已验证发现不懂）
> - **前置知识补丁**：本主题大纲外、被动补充的前置概念（被动补充 + 大纲外）
>
> C型卡点记录规则：前置概念是"本主题学过的" → 只记录"搁置卡点"；前置概念是"从未涉及的" → 记录"前置知识补丁"，若仍无法推进同时记录"搁置卡点"。

## 理解债务

> 学生声称理解但未经验证的概念点。每次开场时优先验证最近 1-2 条。

| 日期 | 阶段 | 概念 | 状态 |
|---|---|---|---|
| YYYY-MM-DD | stage-NN | 概念名 | 未验证 / 已验证 ✓ / 验证发现卡点（见搁置卡点）|

## 搁置卡点

> 当前无法突破、暂时跳过的卡点，下一阶段开始时重新尝试。

| 日期 | 阶段 | 卡点描述 | 诊断类型 | 处理状态 |
|---|---|---|---|---|
| YYYY-MM-DD | stage-NN | 描述 | A/B/C/D 型 | 搁置中 / 已解决 |

## 前置知识补丁

> C 型卡点中被动补充的前置概念——本主题从未正式覆盖、临时给出的知识片段，待后续系统补全。

| 日期 | 阶段 | 前置概念 | 状态 |
|---|---|---|---|
| YYYY-MM-DD | stage-NN | 概念名 | 待补全 / 已在 stage-XX 补全 ✓ |

## 待回答问题

> 学生提出的超出当前阶段的好问题，记录以便后续回答。

- [YYYY-MM-DD stage-NN] 问题内容 → 计划在 stage-XX 回答
```
